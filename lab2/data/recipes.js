...
const updateScore = async (recipeId) => {
    ...
    if (existsInScoreBoard !== null) {
        ...
        await client.zIncrBy('mostaccessed', 1, recipeId);
        console.log('found recipe in sorted set, increase score by 1');
    } else {
        ...
        console.log('set current recipe score to be 1 in sorted set')
    }
}
...
const exportedMethods = {
    ...
    async getRecipeByIdAndStoreInRedis(recipeId) {
        const recipeCollection = await recipes();
        let recipe = await recipeCollection.findOne({ _id: ObjectId(recipeId) });
        if (recipe === null) throw 'No recipe with that id';
        const str_recipe = JSON.stringify(recipe)
        await client.set(recipeId, str_recipe);
        updateScore(recipeId)
        console.log("Recipe with id:" + recipeId + " has stored/reset in redies")
        return recipe;
    },
    ...
    async createRecipe(title, ingredients, cookingSkillRequired, steps, userThatPosted) {
        ...
        return await this.getRecipeByIdAndStoreInRedis(insertInfo.insertedId.toString())
    },
    ...
    async updateRecipe(recipeId, actionUserID, updateFields) {
        ...
        if (!updateInfo.matchedCount && !updateInfo.modifiedCount)
            throw 'Update failed';
        return await this.getRecipeByIdAndStoreInRedis(recipeId);
    },
    ...
    async deleteCommentFromRecipe(recipeId, commentIdToDelete, actionUserId) {
        ...
        recipe.comments.forEach(comment => {
            if (comment._id != commentIdToDelete) {
                ...
                if (actionUserId != comment.userThatPostedComment._id) {
